...
academic_unique = academic[academic['year']==2025]
academic_unique.head()



merge_schools = pd.merge(academic, school_loc_unique, left_on='school_name', right_on='NAME', how='left')

merge_schools.columns

merge_schools.head()
len(merge_schools)
merge_schools.to_csv('merge_schools.csv')
...
demo_aca_drop = demo_aca_drop
cook_clean_unique = cook_clean.copy()
...
#graphs
school_loc.head()
school_loc_geo = gpd.GeoDataFrame(
    school_loc,
    geometry=[Point(xy) for xy in zip(school_loc['LON'], school_loc['LAT'])],
    crs="EPSG:4326"  # WGS84 lat/lon
)

school_loc_geo.crs
school_loc_geo = school_loc_geo.to_crs(epsg=3857)

desert_merge = desert_merge.to_crs(epsg=3857)
desert_merge.head()

fig, ax = plt.subplots(figsize=(10, 10))

# cook_income.plot(
#     column='median_hh_income',   # color by this column
#     scheme='Quantiles',
#     k=7,
#     cmap='viridis',                     # choose your colormap
#     legend=False,                        # show legend
#     ax=ax,
#     edgecolor='black',                  # boundaries color
#     linewidth=0.5
# )
desert_merge.plot(
    column='',   # color by this column
    scheme='Quantiles',
    k=7,
    cmap='viridis',                     # choose your colormap
    legend=False,                        # show legend
