...
demographic data is used only to add a join key (clean school name), that is used for a future merge with school_loc
```{python}
...
merge segregation data with acacdemic data. Then merge this with demographic data to get the correct school names
```{python}
...
add lat/lon values for missing school geometries
```{python}
